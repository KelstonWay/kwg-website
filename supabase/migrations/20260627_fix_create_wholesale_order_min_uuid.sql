-- Fix create_wholesale_order: 20260620_order_rpc_visible_guard introduced the org-resolution
-- aggregate min(r.org_id), but Postgres has no min() aggregate for the uuid type, so EVERY
-- checkout raised 'function min(uuid) does not exist' and /api/submit-order returned 500.
-- We only need a single representative org_id (the query already requires count(distinct)=1),
-- so aggregate over the text representation and cast back to uuid.
--
-- Only line 44's min(r.org_id) -> min(r.org_id::text)::uuid changed; the rest is identical to
-- 20260620_order_rpc_visible_guard.sql.
