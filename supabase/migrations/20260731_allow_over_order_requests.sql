-- Allow buyers to request more than the published quantity (Samuel, 2026-07-31).
--
-- Before: create_wholesale_order hard-rejected any line where qty_requested exceeded
-- availability_release_items.qty_available ('oversold:<id>'), so a buyer could never ask
-- for more than the list showed.
--
-- After: a website order is a REQUEST the team reviews, not a reservation. The full
-- requested quantity is still recorded on wholesale_order_items; the stock decrement is
-- floored at zero so availability never goes negative and a second buyer never sees
-- phantom stock. The website warns the buyer on every over-quantity line, and the
-- internal order email flags those lines for the team.
--
-- The website_visible guard is kept: an item that is hidden or gone still rejects, now as
-- 'unavailable:<id>'. Body otherwise identical to 20260701220500 (org stamp + min-uuid fix).
...
begin
  ...
  loop
    ...
    -- Over-ordering allowed: take what is there, never go below zero.
    update public.availability_release_items
    set qty_available = greatest(qty_available - v_qty_requested, 0)
    where id = v_item_id
      and website_visible;

    get diagnostics v_updated = row_count;
    if v_updated = 0 then
      raise exception 'unavailable:%', v_item_id;
    end if;
  end loop;
