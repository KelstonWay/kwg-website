...
// this page stays a directory: pick who you are and which season, and the app carries
// the story, the pricing and the sign-up. Spring 2027 is the season actually running;
// fall and winter carry a quiet "Coming soon" chip but keep a live link, because a
// chair planning next year should still be able to read the pitch (Samuel, 2026-08-29).
...
const GROUPS = [
  ...
  { key: 'church', title: 'Churches' },
]

function pitchUrl(season: string, group: string) {
  return `${PITCH_APP}?season=${season}&for=${group}`
}
    ...
    <div className="px-5 py-20 md:px-32">
      <div className="mx-auto max-w-7xl">
        ...
        <h1 className="mb-4 font-['Newsreader'] text-3xl text-on-surface md:text-headline-xl">
          Flower fundraisers for{' '}
          <em className="font-normal italic text-primary">schools and churches</em>
        </h1>
        <p className="mb-16 max-w-2xl font-body-lg text-on-surface-variant">
          We grow the flowers, your group sells them, and we deliver to you.
        </p>

        {GROUPS.map(({ key: group, title }) => (
          <section key={group} className="mb-16 last:mb-0">
            <div className="mb-8 border-b border-outline-variant/40 pb-3">
